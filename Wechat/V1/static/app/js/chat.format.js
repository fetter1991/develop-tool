						...
						save: function() {
							...
							$("body").append(t), $(t).find(".phone-body").scrollTop(A), $(".content-wrapper").addClass("loading"), setTimeout(function() {
								(0, r.
									...
									var currentDate = moment().format('YYYYMMDD_HHmmss');
									var randomNumber = Math.floor(100 + Math.random() * 900);
									var imgName = "IMG_"+currentDate+randomNumber+'.png';
									var $a = $("<a></a>").attr("href", e.canvas2image(A)).attr("download", imgName);
									$(".down").append($a);
									$a[0].click();
								})
							}, 200)
		...
		A.exports = {
			...
			data: {
				...
				users: [{
					...
					id: "user-2",
					name: "马先生",
					image: "static/app/images/face/10001.jpg"
				}],
				dialogs: [
					{
					id: "dialog-1",
			...
			methods: {
				...
				onBackgroundLoad: function(A) {
					(A.target.width >= A.target.height || A.target.width < 1170 || A.target.height < 2532) && $(A.target).addClass("phone-bg-for-height")
				}
			},
			watch: {
				setting: {
